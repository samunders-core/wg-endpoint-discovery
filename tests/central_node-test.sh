...
it_requires_central_node_address_as_argument() {
...
it_excludes_itself_from_served_online_peers() {
    (
        ...
        echo "echo 'vpn1	private_key	central_node_public_key	1234	off'"
...
it_serves_endpoint() {
    ...
    PATH=$(pwd) ./redbean.com 127.0.0.1 &
    sleep 1
    OUTPUT="$(curl http://localhost:8080/endpoint/central_node_public_key)"
